...
var lightAmbient2 = vec4(0, 0, 0, 1.0 );     //全局的光
var lightDiffuse2 = vec4( 0, 0, 0, 1.0);    //漫反射
var lightSpecular2 = vec4( 0, 0, 0.0, 1.0 );  //镜面反射
...
var CubeRotateAngle = 0.0; //海绵宝宝旋转角度
...
window.onload = function init() {
    ...
    gl.vertexAttribPointer( vNormal, 4, gl.FLOAT, false, 0, 0 );
    gl.enableVertexAttribArray( vNormal );
    ambientProduct = mult(lightAmbient, vec4(1,1,214.0/255,1.0));//rgba(255,255,214,1.0)   
...
//function drawPants(a, b, c, d, colorIndex, points, colors) 
    ...
    drawBody(0, 1, 2, 3, 3, points2, colors2); // 身体的第一个面，粉色
    drawBody(0, 3, 7, 4, 3, points2, colors2); // 身体的第二个面，粉色
    drawBody(4, 5, 6, 7, 3, points2, colors2); // 身体的第三个面，粉色
    drawBody(1, 5, 6, 2, 3, points2, colors2); // 身体的第四个面，粉色
    drawBody(0, 4, 5, 1, 3, points2, colors2); // 身体的第五个面，粉色
    drawBody(3, 7, 6, 2, 3, points2, colors2); // 身体的第六个面，粉色

    drawCloth(0, 1, 2, 3, 3, points2, colors2); // 衣服的第一个面，白色
    drawCloth(0, 3, 7, 4, 3, points2, colors2); // 衣服的第二个面，白色
    drawCloth(4, 5, 6, 7, 3, points2, colors2); // 衣服的第三个面，白色
    drawCloth(1, 5, 6, 2, 3, points2, colors2); // 衣服的第四个面，白色
    drawCloth(0, 4, 5, 1, 3, points2, colors2); // 衣服的第五个面，白色
    drawCloth(3, 7, 6, 2, 3, points2, colors2); // 衣服的第六个面，白色
...
function drawLeftLeg(a, b, c, d, colorIndex, points, colors) {
    ...
    console.log("leftLegVertices0：",leftLegVertices[0])
    console.log("leftLegVertices1：",leftLegVertices[1])
    console.log("leftLegVertices2：",leftLegVertices[2])
    console.log("leftLegVertices3：",leftLegVertices[3])
    console.log("leftLegVertices4：",leftLegVertices[4])
    console.log("leftLegVertices5：",leftLegVertices[5])
    console.log("leftLegVertices6：",leftLegVertices[6])
    console.log("leftLegVertices7：",leftLegVertices[7])
    console.log("leftLegVertices8：",leftLegVertices[8])
    var indices = [a, b, c, a, c, d]; // 顶点索引顺序
    // 存取顶点余顶点索引信息算法
    for (var i = 0; i < indices.length; i++) {
