...
function setPoints() {
    // 画海绵宝宝
    drawMouse(points, colors, 0,0.0, 0.24, 0.1019);
    ...
    // 画粉色海绵宝宝
    drawMouse(points2, colors2, 0,0.0, 0.24, 0.1019);
...
function drawLeftEye(points, colors) {
    ...
    console.log("眼白：",leftEyeVertices.length);
...
function drawMouse(points, colors, colorIndex,x,y,z) {
    ...
    mouseVertices = getCircleVertex(x, y, z, 0.205, ms, 80, 140);
    for (var i = 0; i < mouseVertices.length; i++) {
        points.push(mouseVertices[i]);
        colors.push(chooseColors[colorIndex]); // 黄色
    }
